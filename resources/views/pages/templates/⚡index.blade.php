...
<div class="flex h-full w-full flex-1 flex-col gap-8" data-view="templates-index">
    ...
    <div class="flex flex-col gap-3">
        <div class="flex items-center justify-between">
            ...
            <flux:modal.trigger name="create-feature-template">
        ...
        <div class="space-y-2">
            @forelse ($this->featureTemplates as $template)
                <div wire:key="feature-template-{{ $template->id }}" class="flex items-center justify-between rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <div class="flex items-center gap-1">
                        <flux:tooltip :content="__('Edit')">
                            <flux:button variant="ghost" size="sm" icon="pencil" :href="route('templates.feature', $template)" wire:navigate />
                        </flux:tooltip>
                        <flux:tooltip :content="__('Delete')">
                            <flux:button variant="ghost" size="sm" icon="trash" wire:click="confirmDeleteFeatureTemplate('{{ $template->id }}')" />
                        </flux:tooltip>
                    </div>
                </div>
            @empty
    ...
    <div class="flex flex-col gap-3">
        <div class="flex items-center justify-between">
            ...
            <flux:modal.trigger name="create-epic-template">
        ...
        <div class="space-y-2">
            @forelse ($this->epicTemplates as $template)
                <div wire:key="epic-template-{{ $template->id }}" class="flex items-center justify-between rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                    <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-1">
                        <flux:tooltip :content="__('Edit')">
                            <flux:button variant="ghost" size="sm" icon="pencil" :href="route('templates.epic', $template)" wire:navigate />
                        </flux:tooltip>
                        <flux:tooltip :content="__('Delete')">
                            <flux:button variant="ghost" size="sm" icon="trash" wire:click="confirmDeleteEpicTemplate('{{ $template->id }}')" />
                        </flux:tooltip>
                    </div>
                </div>
            @empty
